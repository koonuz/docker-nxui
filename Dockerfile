...
ENV GET_ARCH amd64
RUN apk update && \
    apk add --no-cache tzdata runit bash ca-certificates && \
    cp /usr/share/zoneinfo/Asia/Shanghai /etc/localtime && \
    echo "Asia/Shanghai" > /etc/timezone && \
    mkdir /lib64 && ln -s /lib/libc.musl-x86_64.so.1 /lib64/ld-linux-x86-64.so.2 && \
    wget -N --no-check-certificate -P /usr/local -O x-ui-linux-${GET_ARCH}.tar.gz https://github.com/FranzKafkaYu/x-ui/releases/download/${GET_VERSION}/x-ui-linux-${GET_ARCH}.tar.gz && \
